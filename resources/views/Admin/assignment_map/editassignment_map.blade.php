	...
	<div class="row">
        ...
        <div class="col-xl-12 col-lg-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form  method="post" action="{{ url('Admin/edit-assignment_map') }}/{{ $tabledata->id }}" >
                        @csrf
                    	<div class="row">
							...
							<div class="col-md-4">            		 	
                    	        ...
                    	        <select name="client_id"  class="form-control">
                    	            ...
                    	            <option value="{{ $clients->id }}" @if($tabledata->client_id == $clients->id) selected @endif required>{{ $clients->name }}</option>
                                    @endforeach
                    	        </select>
                		 	</div>
                            ...
                            <div class="col-md-4">            		 	
                    	        ...
                    	        <select name="assignemnt_id"  class="form-control">
                    	            ...
                    	            <option value="{{ $assignments->id }}" @if($tabledata->assignemnt_id == $assignments->id) selected @endif required>{{ $assignments->name }}</option>
                                    @endforeach
                    	        </select>
                		 	</div>
							 ...
							 <div class="col-md-4">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
		                                <input type="date" name="period_end" value="{{ $tabledata->period_end }}" class="form-control" required>
		                            </div>
		                        </div>
                		 	</div>
                            ...
                            <div class="col-md-4">            		 	
                    	        ...
                    	        <select name="period"  class="form-control">
                    	            <option value="none" @if($tabledata->period == "none") selected @endif >Period Type</option>
                    	            <option value="Annual" @if($tabledata->period == "Annual") selected @endif >Annual</option>
                    	            <option value="Monthly" @if($tabledata->period == "Monthly") selected @endif >Monthly</option>
                    	        </select>
                		 	</div>
							...
							<div class="col-md-4">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
		                                <input type="text" name="total_agreed_fees" value="{{ $tabledata->total_agreed_fees }}" id="agreed_fees" oninput="set_agreed_fees()" class="form-control" required>
		                            </div>
		                        </div>
                		 	</div>
							...
							<div class="row mt-3 table_head_row table-responsive">
								...
								<table border="1px solid black" class="table table-bordered table-striped" cellspacing="0" width="70%" id="table_tg" >
									...
									<tbody class="table tbody">
                                        ...
                                        @foreach($AM_Employee as $key => $AM_Employees)
                                            <?php $i = $key+1; ?>
                                            <tr class="table tr" id="row_{{ $i }}">
                                                <td class="table td">
                                                    <select name="employee_id[]"  class="form-control" id="employee_{{ $i }}" onchange="fetchemployee({{ $i }})">
                                                        <?php $employee = DB::table('employee')->get(); ?>
                                                        <option value="" required>Select Employee</option>
                                                        @foreach($employee as $employees)
                                                        <option value="{{ $employees->id }}" @if($AM_Employees->employee_id == $employees->id) selected @endif required>{{ $employees->name }}</option>
                                                        @endforeach
                                                    </select>
                                            		<input type="hidden" value="{{ $AM_Employees->id }}" name="AM_Employees_id[]" >
                                                    <input type="hidden" id="hourly_{{ $i }}" name="hourly[]" value="">
                                                    <input type="hidden" id="rc_hourly_{{ $i }}" name="rc_hourly[]" value="">
                                                </td>

                                                <td class="table td"><input type="date" id="fromdate_{{ $i }}" value="{{ $AM_Employees->from_date }}" name="from_date[]"></td>
                                                <td class="table td"><input type="date" id="todate_{{ $i }}" value="{{ $AM_Employees->to_date }}" name="to_date[]"></td>
                                                <td class="table td"><input type="text" id="estimate_{{ $i }}" value="{{ $AM_Employees->hours }}" oninput="calculateTotal({{ $i }})" name="hours[]"></td>
                                                <td class="table td"><input type="text" id="recoverable_c_{{ $i }}" value="{{ $AM_Employees->rc_cost }}" readonly name="rc_cost[]"></td>
                                                <td class="table td"><input type="text" id="actual_c_{{ $i }}" readonly value="{{ $AM_Employees->actual_cost }}" name="actual_cost[]"></td>
                                                <td class="table td"><input type="checkbox" id="tl_{{ $i }}" name="tl[]" @if($AM_Employees->tl == "1" ) checked @endif value="1"><span>Yes</span></td>
                                                <td class="table td"><input type="text" id="agreed_{{ $i }}" oninput="set_agreed_fees()" value="{{ $AM_Employees->agreed_fees }}" name="agreed_fees[]"></td>
                                                <td class="table td"><i class="fa fa-trash cur_poi" onclick="del_fun({{ $i }})"></i></td>
                                            </tr>
                                        @endforeach
									</tbody>
							...
							<div class="col-md-3">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
                                        <input type="hidden" value="{{ $AM_Partner?$AM_Partner->id:'' }}" name="AM_Partner_id">
		                                <input type="text" id="total_recoverable_c" name="total_recoverable_c" value="{{ $AM_Partner?$AM_Partner->total_recoverable_c:'' }}" class="form-control" onclick="set_oth_val()" readonly required>
		                            </div>
		                        </div>
                		 	</div>
							<div class="col-md-3">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
		                                <input type="text" name="recovery_margin" value="{{ $AM_Partner?$AM_Partner->recovery_margin:'' }}" id="recovery_margin" class="form-control" readonly required>
		                            </div>
		                        </div>
                		 	</div>
							 <div class="col-md-3">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
		                                <input type="text" name="total_actual_c" value="{{ $AM_Partner?$AM_Partner->total_actual_c:'' }}" id="total_actual_c" class="form-control" readonly required>
		                            </div>
		                        </div>
                		 	</div>
							 <div class="col-md-3">            		 	
		                        ...
		                        <div class="form-group">
		                            <div class="form-line">
		                                <input type="text" name="actual_margin" value="{{ $AM_Partner?$AM_Partner->actual_margin:'' }}" id="actual_margin" class="form-control" readonly required>
		                            </div>
		                        </div>
                		 	</div>
							 ...
							 <div class="col-md-3">            		 	
								...
								<select name="bd"  class="form-control">
									<option value="" @if($AM_Partner) @if($AM_Partner->bd == "") selected @endif @endif required>BD Partner</option>
									@foreach($employeeBD as $employeeBDs)
									<option value="{{ $employeeBDs->id }}" @if($AM_Partner) @if($AM_Partner->bd == $employeeBDs->id) selected @endif @endif required>{{ $employeeBDs->name }}</option>
									@endforeach
								</select>
							 ...
							 <div class="col-md-3">            		 	
								...
								<select name="co_bd_1"  class="form-control">
									...
									<option value="{{ $employeexceptBds->id }}" @if($AM_Partner) @if($AM_Partner->bd == $employeexceptBds->id) selected @endif @endif required>{{ $employeexceptBds->name }}</option>
									@endforeach
								</select>
							...
							<div class="col-md-6">
                    	        ...
                    	        <select name="status" required class="form-control">
                    	            <option value="" @if($AM_Partner?$AM_Partner->status:'' == "") selected @endif >Select User Status</option>
                    	            <option value="0" @if($AM_Partner?$AM_Partner->status:'' == "0") selected @endif >Active</option>
                    	            <option value="1" @if($AM_Partner?$AM_Partner->status:'' == "1") selected @endif >In-Active</option>
                    	        </select>
                		 	</div>
